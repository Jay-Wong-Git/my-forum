...
public class ForumController {
    ...
    public RestBean<WeatherVO> weather(double longitude, double latitude) {
        ...
        if (vo == null) {
            return RestBean.failure(400, "Failed to get weather information.");
        } else {
    ...
    public RestBean<Void> createTopic(@Valid @RequestBody TopicCreateVO vo,
        ...
        return controllerUtils.messageHandler(() -> topicService.createTopic(id, vo));
    }

    @PostMapping("/update-topic")
    public RestBean<Void> updateTopic(@Valid @RequestBody TopicUpdateVO vo,
                                      @RequestAttribute(Const.ATTR_USER_ID) int id) {
        return controllerUtils.messageHandler(() -> topicService.updateTopic(id, vo));
    }
    ...
    public RestBean<Void> addComment(@Valid @RequestBody AddCommentVO vo,
                                     @RequestAttribute(Const.ATTR_USER_ID) int id) {
        return controllerUtils.messageHandler(() -> topicService.createComment(id, vo));
    }
